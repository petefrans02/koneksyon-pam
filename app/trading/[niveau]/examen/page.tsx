...
export async function generateMetadata({ params }: { params: Promise<{ niveau: string }> }) {
  ...
  if (!lvl) return {};
  return {
    title: `Examen — ${lvl.title.fr}`,
    description: `Examen de fin de niveau ${lvl.n}. Seuil de réussite : ${lvl.passingScore}%.`,
    alternates: alternates(`/trading/${niveau}/examen`),
  };
}
...
export default async function Page({ params }: { params: Promise<{ niveau: string }> }) {
  ...
  const lvl = getLevel(niveau);
  if (!lvl) notFound();
  return <QuizGate niveau={lvl} questions={questionsForLevel(niveau)} mode="examen" />;
}
