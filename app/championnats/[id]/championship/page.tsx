  ...
  return (
    <div key={streak} className="fixed top-16 left-1/2 -translate-x-1/2 z-40 pointer-events-none"
      ...
      <div className="flex items-center gap-2 px-4 py-1.5 rounded-full border text-sm font-black"
        ...
        {Array.from({ length: flameCount }).map((_, i) => <Icon key={i} name="feu" size={14} />)} {streak}x {label}
      </div>
    </div>
  );
}
...
export default function ChampionshipPage() {
  ...
  const handleStateChange = useCallback((row: ChampionshipStateRow) => {
    ...
    // Suspense delay before reveal (court, pour laisser le maximum de temps à la réponse)
    if (phaseChanged && row.phase === "ANSWER_REVEAL") {
      setRevealReady(false);
      setTimeout(() => setRevealReady(true), 450);
    }

    // Routing
    if (row.phase === "LOBBY") { router.push(`/championnats/${id}/lobby`); return; }
    if (row.phase === "PODIUM") { stopMusic(2); if (audioEnabled) playVictoryFanfare(); router.push(`/championnats/${id}/podium`); return; }
    if (row.phase === "CLOSED" || row.phase === "CANCELLED") { stopMusic(1.2); router.push("/dashboard"); return; }
  ...
  useEffect(() => {
    ...
    fetch(`/api/engine/${id}/state`)
      ...
      .catch(() => router.push(`/championnats/${id}/lobby`));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [id]);
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      // Si le reveal arrive tard chez un client, on force l'affichage de la réponse
      // avant que la phase n'avance (sinon la bonne réponse ne s'affiche jamais).
      if (state.phase === "ANSWER_REVEAL" && rem < 1200) setRevealReady(true);
  ...
  if (isPaused) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6" style={{ background: "#020717" }}>
        <style>{CSS}</style>
        <div className="animate-pulse flex justify-center text-white"><Icon name="chrono" size={60} /></div>
  ...
  if (state.phase === "COUNTDOWN") {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-8" style={{ background: "#020717" }}>
        ...
        <div key={sec} className="text-9xl font-black" style={{ animation: "champ-count 1s ease forwards", color: sec <= 3 ? "#ef4444" : "#f59e0b", textShadow: `0 0 60px ${sec <= 3 ? "#ef4444" : "#f59e0b"}60` }}>
          {sec <= 0 ? <Icon name="feu" size={96} /> : sec}
        </div>
        <p className="text-white/20 text-sm">{state.player_count} {l === "fr" ? "joueurs" : l === "ht" ? "jwè" : l === "es" ? "jugadores" : "players"}</p>
        {!audioEnabled && (
          <button onClick={enableAudio} className="mt-4 px-5 py-2 rounded-full border border-white/10 text-white/40 text-xs hover:text-white/70 transition-colors inline-flex items-center gap-1.5">
            <Icon name="audio" size={16} /> {l === "fr" ? "Activer le son" : l === "ht" ? "Aktive son an" : l === "es" ? "Activar el sonido" : "Enable sound"}
          </button>
        )}
      ...
      return (
        <div className="min-h-screen flex flex-col items-center justify-center gap-8 px-4 text-center relative overflow-hidden"
          ...
          <div className="flex justify-center" style={{ animation: "finale-crown 0.9s 0.2s cubic-bezier(0.34,1.56,0.64,1) both", opacity: 0, animationFillMode: "forwards", filter: "drop-shadow(0 0 30px rgba(245,158,11,0.60))" }}>
            <Icon name="couronne" size={64} color="#f59e0b" />
          </div>
          ...
          <div className="w-full max-w-sm space-y-2">
            ...
            {top3.map((p, i) => (
              <div key={i} className="flex items-center gap-3 rounded-2xl px-4 py-3 border border-white/5" style={{ background: "rgba(255,255,255,0.02)" }}>
                <span className="inline-flex"><Icon name="medaille" size={18} color={i === 0 ? "#f59e0b" : i === 1 ? "#9ca3af" : "#b45309"} /></span>
                {p.player_avatar
    ...
    if (!revealReady) {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-4" style={{ background: "#020717" }}>
          <style>{CSS}</style>
          <div className="flex justify-center text-white/80" style={{ animation: "champ-pulse 0.6s ease-in-out infinite" }}><Icon name="idee" size={48} /></div>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center gap-5 px-4 py-10" style={{ background: "#020717" }}>
        ...
        {didAnswer && (
          <div className="w-full max-w-sm rounded-3xl p-5 text-center border" style={{
            ...
            {streak >= 3 && isCorrect && (
              <p className="text-amber-400 text-xs font-black mt-1 flex items-center justify-center gap-1"><Icon name="feu" size={13} /> {streak}x {l === "fr" ? "en série !" : l === "ht" ? "an seri !" : l === "es" ? "en racha !" : "streak!"}</p>
            )}
          </div>
        )}
        {!didAnswer && (
          <div className="w-full max-w-sm rounded-3xl p-5 text-center border border-amber-400/20" style={{ background: "rgba(245,158,11,0.06)", animation: "champ-fade 0.4s ease both" }}>
            <div className="mb-2 flex justify-center text-amber-400"><Icon name="chrono" size={36} /></div>
  ...
  if (state.phase === "QUESTION_ACTIVE") {
    ...
    function renderAnswers() {
      ...
      if (qType === "tf") {
        const btns = [
          { idx: 0, label: l === "fr" ? "Vrai" : l === "ht" ? "Vre" : l === "es" ? "Verdadero" : "True", color: "#22c55e" },
          { idx: 1, label: l === "fr" ? "Faux" : l === "ht" ? "Fo" : l === "es" ? "Falso" : "False", color: "#ef4444" },
        ];
        return (
            ...
            {btns.map(btn => {
              ...
              return (
                <button key={btn.idx} onClick={() => submitAnswer(btn.idx)}
                  ...
                  className="py-6 rounded-3xl border font-black text-base transition-all active:scale-95 disabled:cursor-default flex items-center justify-center gap-2"
                  style={{
                  ...
                  <Icon name={btn.idx === 0 ? "succes" : "fermer"} size={18} /> {btn.label}
                </button>
              );
            })}
          </div>
        ...
        <div className="sticky top-0 z-40 border-b border-white/5" style={{ background: "rgba(2,7,23,0.95)", backdropFilter: "blur(12px)" }}>
          <div className="max-w-lg mx-auto px-4 h-14 flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 text-xs">
              <span className="text-white/20 inline-flex"><Icon name="etoile" size={16} /></span>
              <AnimatedScore score={myScore} prevScore={prevScore} />
            </div>
            {myRank && <span className="text-amber-400/60 text-xs font-black">#{myRank}</span>}
            {!audioEnabled ? (
              <button onClick={enableAudio} className="text-white/20 hover:text-white/50 px-2 py-1 rounded-lg border border-white/10 transition-colors inline-flex items-center"><Icon name="audio" size={15} /></button>
            ) : (
              <button onClick={() => setShowAudioPanel(p => !p)} className="text-white/40 hover:text-white/70 px-2 py-1 rounded-lg border border-white/10 transition-colors inline-flex items-center" title={l === "fr" ? "Réglages audio" : l === "ht" ? "Réglaj son" : l === "es" ? "Configuración de audio" : "Audio settings"}>
                <Icon name="musique" size={15} />
              </button>
            )}
        ...
        {showQuitConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: "rgba(2,7,23,0.88)", backdropFilter: "blur(8px)" }}>
            <div className="rounded-3xl border border-white/10 p-8 text-center max-w-xs w-full" style={{ background: "rgba(255,255,255,0.06)" }}>
              <p className="mb-4 flex justify-center text-white/80"><Icon name="deconnexion" size={36} /></p>
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 py-6 gap-5 max-w-lg mx-auto w-full" style={{ animation: "champ-fade 0.3s ease both" }}>
          ...
          {(qType === "character" || qType === "location" || qType === "book") && (
            <div className="text-center space-y-1">
              ...
              {question?.ref && (
                <p className="text-amber-400/70 text-xs font-semibold tracking-wide flex items-center justify-center gap-1"><Icon name="bible" size={13} /> {t(toLText(question.ref), l)}</p>
              )}
            </div>
          )}
          ...
          {qType !== "character" && qType !== "location" && qType !== "book" && qType !== "fill" && (
            <div className="w-full max-w-sm text-center">
              ...
              {question?.ref && (
                <p className="text-amber-400/70 text-xs mt-2 font-semibold tracking-wide flex items-center justify-center gap-1">
                  <Icon name="bible" size={13} /> {t(toLText(question.ref), l)}
                </p>
              )}
            </div>
          )}
          ...
          {state.round_index === 14 && finaleLb.length > 0 && (
            <div className="w-full max-w-sm rounded-2xl border border-amber-400/15 overflow-hidden" style={{ background: "rgba(245,158,11,0.04)" }}>
              <p className="text-amber-400/40 text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 py-1.5 border-b border-amber-400/10">
                <Icon name="couronne" size={12} /> {l === "fr" ? "Classement en direct" : l === "ht" ? "Klasman an dirèk" : l === "es" ? "Clasificación en vivo" : "Live ranking"}
              </p>
                ...
                {finaleLb.map((entry, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2 px-3 py-2"
                      ...
                      <span className="text-sm w-5 flex justify-center shrink-0">{i < 3 ? <Icon name="medaille" size={15} color={i === 0 ? "#f59e0b" : i === 1 ? "#9ca3af" : "#b45309"} /> : `#${entry.rank}`}</span>
