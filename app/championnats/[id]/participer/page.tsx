...
export default function ParticiperPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (!user) { router.replace(`/auth?redirect=/championnats/${id}/participer`); return; }
      ...
      if (!c) { router.replace("/championnats"); return; }
      setContest(c);
      setLoading(false);

      if (c.status === "active") { launchPlay(); return; }
      if (c.status === "completed") { router.replace(`/championnats/${id}/results`); return; }
    }
    init();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload: { new: Contest }) => {
          ...
          if (updated.status === "completed") router.replace(`/championnats/${id}/results`);
        }
      )
      .subscribe();
    ...
    const poll = setInterval(async () => {
      try {
        ...
        if (status === "completed" && !launchRef.current) router.replace(`/championnats/${id}/results`);
      } catch { /* ignore */ }
    }, 3000);
  ...
  function launchPlay() {
    ...
    setLaunching(true);
    setTimeout(() => router.replace(`/championnats/${id}/play`), 900);
  }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-5 py-10 relative overflow-hidden"
      ...
      {launching && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center"
          ...
          <div className="mb-5" style={{ animation: "kp-spin 1.2s linear infinite",
            filter: "drop-shadow(0 0 20px rgba(197,168,79,0.8))" }}><Icon name="croix" size={64} color="#c5a84f" /></div>
      ...
      <div className="relative z-10 w-full max-w-sm flex flex-col items-center gap-6" style={{ paddingTop: participants.length > 0 ? "3rem" : 0 }}>
        ...
        <div className="relative flex items-center justify-center mb-2"
          ...
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center"
            ...
            <span style={{ filter: "drop-shadow(0 0 10px rgba(197,168,79,0.7))",
              animation: "kp-breathe 2.5s ease-in-out infinite", display: "inline-flex" }}><Icon name="croix" size={30} color="#c5a84f" /></span>
          </div>
        </div>
