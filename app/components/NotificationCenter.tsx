...
export default function NotificationCenter() {
  ...
  function timeAgo(iso: string): string {
    ...
    if (diff < 3600) { const m = Math.floor(diff / 60); return l === "fr" ? `il y a ${m}min` : l === "ht" ? `${m}min` : `${m}m ago`; }
    if (diff < 86400) { const h = Math.floor(diff / 3600); return l === "fr" ? `il y a ${h}h` : l === "ht" ? `${h}h` : `${h}h ago`; }
    const d = Math.floor(diff / 86400); return l === "fr" ? `il y a ${d}j` : l === "ht" ? `${d}j` : `${d}d ago`;
  }
