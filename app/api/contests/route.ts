...
export async function GET(request: NextRequest) {
  ...
  let query = db
    .from("contests")
    .select(`id, title, title_ht, title_en, description, description_ht, description_en, status, start_at, scheduled_start_at, duration_minutes, theme, week_number, season_number, created_at, max_participants, enabled, contest_participants(count), contest_sessions(count)`)
    .order("scheduled_start_at", { ascending: false, nullsFirst: false });

  if (!isAdminUser) {
    query = query.neq("enabled", false); // hide disabled contests from public
  }
