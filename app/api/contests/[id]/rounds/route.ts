...
import { adminDb, isAdminEmail } from "@/lib/admin-auth";
...
export async function GET(
  ...
  // Check if caller is admin — admins always get full questions with correct answers
  ...
  const callerIsAdmin = user?.email ? isAdminEmail(user.email) : false;
  ...
  const safeRounds = rounds.map(r => ({
    ...r,
    questions: (isCompleted || callerIsAdmin)
      ? r.questions
