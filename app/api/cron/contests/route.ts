// Runs daily — auto-starts/closes contests and auto-posts to Facebook
...
export async function GET(request: NextRequest) {
  ...
  for (const contest of toStart ?? []) {
    ...
    // Auto-post to Facebook when a contest goes live
    try {
      const r = await autoPostContest(contest.title, contest.id, "fr");
      if (r.any_success) actions.push(`FB posted: ${contest.title}`);
      else actions.push(`FB failed: ${r.results[0]?.error ?? "unknown"}`);
    } catch (e) { actions.push(`FB error: ${String(e)}`); }
  }
