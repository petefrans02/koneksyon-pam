...
const STATUS_LABEL: Record<string, ReactNode> = {
  completed:         "Confirmé",
  webhook_confirmed: <span className="inline-flex items-center gap-1">Webhook <Icon name="valider" size={13} /></span>,
  pending:           "En attente",
  denied:            "Refusé",
  refunded:          "Remboursé",
};
...
export default function AdminDonsPage() {
  ...
  const statCards = stats ? [
    { label: "Dons au total",    value: `$${fmt(stats.totalAmount)}`,  sub: `${stats.totalCount} transactions`, icon: "don", color: "from-emerald-500 to-teal-600" },
    { label: "Ce mois",          value: `$${fmt(stats.monthAmount)}`,  sub: "dons du mois en cours",           icon: "evenements", color: "from-blue-500 to-indigo-600" },
    { label: "Aujourd'hui",      value: `$${fmt(stats.todayAmount)}`,  sub: "reçus aujourd'hui",               icon: "soleil", color: "from-amber-500 to-orange-500" },
    { label: "Taux confirmation",
      value: stats.totalCount > 0
        ? `${Math.round(((stats.byStatus.webhook_confirmed ?? 0) / stats.totalCount) * 100)}%`
        : "—",
      sub: "webhooks PayPal confirmés", icon: "valider", color: "from-violet-500 to-purple-600" },
  ] : [];
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-bold text-stone-800 flex items-center gap-2">
              <Icon name="don" size={20} /> Tableau de bord des dons
            </h1>
          </div>
          <div className="flex gap-3">
            ...
            >
              {exporting ? "…" : <Icon name="telechargement" size={16} />} Export CSV
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden">
          ...
          {loading && rows.length === 0 ? (
            ...
            <div className="py-16 text-center">
              <div className="text-4xl mb-3"><Icon name="foi" size={28} /></div>
                    ...
                    <tr key={row.id} className="hover:bg-stone-50 transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        {row.webhook_confirmed_at ? (
                          <span className="text-emerald-600 font-semibold text-xs inline-flex items-center gap-1"><Icon name="valider" size={13} /> {fmtDate(row.webhook_confirmed_at)}</span>
                        ) : (
