...
function PodiumEntry({ entry, position }: { entry: LeaderboardEntry; position: 1 | 2 | 3 }) {
  const config = {
    1: { height: "h-36", bg: "bg-gradient-to-b from-[#c5a84f]/20 to-transparent", border: "border-[#c5a84f]/40", nameSize: "text-base", glow: "0 0 30px rgba(197,168,79,0.18)", crown: "couronne" as IconName, crownColor: "#c5a84f" },
    2: { height: "h-24", bg: "bg-gradient-to-b from-white/8 to-transparent", border: "border-white/20", nameSize: "text-sm", glow: undefined, crown: "medaille" as IconName, crownColor: "#c0c0c0" },
    3: { height: "h-20", bg: "bg-gradient-to-b from-amber-700/15 to-transparent", border: "border-amber-700/30", nameSize: "text-sm", glow: undefined, crown: "medaille" as IconName, crownColor: "#cd7f32" },
  }[position];

  return (
    <div className="flex flex-col items-center gap-2 flex-1">
      <Icon name={config.crown} size={18} color={config.crownColor} />
                ...
                <div key={entry.user_id}
                  ...
                  <div className="flex justify-center">
                    <span className={`text-sm font-black tabular-nums inline-flex items-center ${
                      ...
                      {actualRank <= 3
                        ? <Icon name={actualRank === 1 ? "couronne" : "medaille"} size={16} color={actualRank === 1 ? "#c5a84f" : actualRank === 2 ? "#c0c0c0" : "#cd7f32"} />
                        : `#${actualRank}`}
                    </span>
                  </div>
