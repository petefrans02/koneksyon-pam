...
function PodiumEntry({ entry, position }: { entry: LeaderboardEntry; position: 1 | 2 | 3 }) {
  const config = {
    1: { height: "h-36", bg: "bg-gradient-to-b from-[#c5a84f]/20 to-transparent", border: "border-[#c5a84f]/40", nameSize: "text-base", glow: "0 0 30px rgba(197,168,79,0.18)", crown: "👑" },
    2: { height: "h-24", bg: "bg-gradient-to-b from-white/8 to-transparent", border: "border-white/20", nameSize: "text-sm", glow: undefined, crown: "🥈" },
    3: { height: "h-20", bg: "bg-gradient-to-b from-amber-700/15 to-transparent", border: "border-amber-700/30", nameSize: "text-sm", glow: undefined, crown: "🥉" },
  }[position];

  return (
    <div className="flex flex-col items-center gap-2 flex-1">
      <p className="text-xs">{config.crown}</p>
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (rd?.leaderboard && rd.session) {
        setMyEntry({ rank: rd.leaderboard.rank_global, user_id: myUserId ?? "", name: "Moi", score: rd.session.score, ...rd.leaderboard });
      }
      setLoading(false);
    });
  }, [id, filter, myUserId]);
    ...
    <div className="min-h-screen" style={{ background: "#030918" }}>
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 py-12">
        ...
        <div className="flex items-center gap-3 mb-2">
          <Link href={`/concours/${id}`} className="text-white/25 hover:text-white/60 transition-colors text-xs font-bold">←</Link>
        ...
        <div className="flex gap-2 mb-10">
          {(["global", "country"] as const).map(f => (
            <button key={f} onClick={() => { setFilter(f); setPage(1); }}
              ...
              {f === "global"
                ? (l === "fr" ? "Mondial" : l === "ht" ? "Mondyal" : "Global")
                : (l === "fr" ? "Mon pays" : l === "ht" ? "Peyi mwen" : "My country")}
            </button>
          ))}
          <span className="ml-auto text-white/20 text-xs self-center">{entries.length} {l === "fr" ? "joueurs" : l === "ht" ? "jwè" : "players"}</span>
        </div>
        ...
        {myEntry && myEntry.rank > 3 && (
          <div className="mb-6 bg-[#c5a84f]/10 border border-[#c5a84f]/25 rounded-2xl px-5 py-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white font-black text-sm">{l === "fr" ? "Votre position" : l === "ht" ? "Pozisyon ou" : "Your position"}</p>
              <p className="text-[#c5a84f] text-xs">{myEntry.score} pts · {myEntry.accuracy_pct ?? 0}% précision</p>
            </div>
            <Link href={`/concours/${id}/results`} className="text-[#c5a84f] text-xs font-black shrink-0">
              Détails →
            </Link>
          </div>
        )}
        ...
        {entries.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-white/25 text-sm">
              {l === "fr" ? "Le classement n'est pas encore disponible." : l === "ht" ? "Klasman an pa disponib ankò." : "Leaderboard not available yet."}
            </p>
          </div>
        ) : (
          <div className="rounded-2xl overflow-hidden border border-white/8">
            ...
            <div className="grid grid-cols-[2.5rem_1fr_5rem_4rem_4rem] gap-0 px-4 py-3 bg-white/[0.03] border-b border-white/8">
              ...
              <span className="text-white/20 text-[10px] font-black uppercase">Joueur</span>
              <span className="text-white/20 text-[10px] font-black uppercase text-right">Points</span>
              <span className="text-white/20 text-[10px] font-black uppercase text-right hidden sm:block">Précis.</span>
              <span className="text-white/20 text-[10px] font-black uppercase text-right hidden sm:block">Temps</span>
            </div>
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className={`text-sm font-bold truncate ${isMe ? "text-[#c5a84f]" : "text-white/80"}`}>
                        {entry.name} {isMe && <span className="text-[10px] text-[#c5a84f]/60">(vous)</span>}
                      </p>
            ...
            {hasMore && (
              <div className="px-4 py-4 flex justify-center border-t border-white/5">
                <button onClick={() => setPage(p => p + 1)}
                  ...
                  {l === "fr" ? "Voir plus" : l === "ht" ? "Wè plis" : "Load more"} ↓
                </button>
              </div>
            )}
          </div>
