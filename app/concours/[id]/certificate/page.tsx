...
export default function CertificatePage() {
  ...
  const medalColor = rank === 1 ? "#c5a84f" : rank === 2 ? "#c0c0c0" : rank === 3 ? "#cd7f32" : "#f59e0b";
    ...
    <div className="min-h-screen flex flex-col items-center justify-start py-8 px-4" style={{ background: "#020717" }}>
      ...
      <div className="relative w-full max-w-sm space-y-6">
        ...
        <div style={{ animation: "cert-rise 0.5s ease both" }}>
          <Link href={`/concours/${id}/podium`} className="text-white/30 hover:text-white/60 text-sm transition-colors">
        ...
        <div ref={certRef} className="rounded-3xl border p-8 text-center relative overflow-hidden"
          ...
          <div className="mb-3 flex justify-center" style={{ animation: "cert-pop 0.6s 0.4s cubic-bezier(0.34,1.56,0.64,1) both", opacity: 0, animationFillMode: "forwards" }}>
            <Icon name="medaille" size={52} color={medalColor} />
          </div>
        ...
        <button onClick={handleShare}
          ...
          {shareSuccess
            ...
            : <span className="inline-flex items-center justify-center gap-2"><Icon name="partage" size={16} />{l === "fr" ? "Partager mon certificat" : l === "ht" ? "Pataje sètifika mwen" : l === "es" ? "Compartir mi certificado" : "Share my certificate"}</span>}
        </button>
        ...
        <div className="flex gap-3" style={{ animation: "cert-rise 0.6s 0.45s ease both", opacity: 0, animationFillMode: "forwards" }}>
          ...
          <Link href="/concours" className="flex-1 py-3 rounded-2xl border border-white/8 text-center text-sm font-bold text-white/50 hover:text-white/80 hover:border-white/20 transition-all">
