...
const RANK_COLORS: Record<number, string> = { 1: "#c5a84f", 2: "#c0c0c0", 3: "#cd7f32" };
...
export default function ResultsPage() {
  ...
  if (!session) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4" style={{ background: "#030918" }}>
      ...
      <Link href={`/concours/${id}`} className="text-[#c5a84f] text-sm font-bold">← {l === "fr" ? "Retour" : l === "ht" ? "Retounen" : l === "es" ? "Volver" : "Back"}</Link>
    </div>
  );
        ...
        <div className={`mb-6 transition-all duration-700 ${shown ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          {isPerfect ? (
            <div className="flex flex-col items-center gap-3">
              <div className="flex justify-center" style={{ animation: "res-bounce .7s .1s ease-out both" }}><Icon name="couronne" size={72} color="#c5a84f" /></div>
            ...
            <div className="flex justify-center" style={{ animation: "res-bounce .7s .1s ease-out both" }}><Icon name="trophee" size={64} color="#c5a84f" /></div>
          )}
        </div>
          ...
          <div className={`w-full mb-6 transition-all duration-700 delay-150 ${shown ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            <div className="flex items-center justify-center gap-6 bg-white/[0.04] border border-white/8 rounded-2xl p-6">
              <div className="text-center">
                <p className="text-white font-black tabular-nums flex items-center justify-center" style={{ fontSize: "2.5rem", lineHeight: 1 }}>
                  {leaderboard.rank_global <= 3
                    ? <Icon name="medaille" size={40} color={RANK_COLORS[leaderboard.rank_global]} />
