...
export default function ContestDetailPage() {
  ...
  if (!contest) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4" style={{ background: "#030918" }}>
      ...
      <Link href="/concours" className="text-[#c5a84f] text-sm font-bold">← {l === "fr" ? "Retour" : l === "ht" ? "Tounen" : l === "es" ? "Volver" : "Back"}</Link>
    </div>
  );
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl mx-auto px-5 py-16 text-center">
            ...
            {alreadyPlayed ? (
              ...
              <Link href={`/concours/${id}/championship`}
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-5 py-16 text-center">
          ...
          <div className="mt-4 pt-4 border-t border-white/8 flex justify-center">
            <Link href={`/private/session/${id}`}
              ...
              <Icon name="utilisateurs" size={16} /> {l === "fr" ? "Jouer avec mes amis →" : l === "ht" ? "Jwe ak zanmi mwen yo →" : l === "es" ? "Jugar con mis amigos →" : "Play with friends →"}
            </Link>
          </div>
        </div>
      </div>
