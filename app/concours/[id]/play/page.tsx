...
function getMedalIcon(accuracy: number): { name: IconName; color: string } {
  if (accuracy === 100) return { name: "medaille", color: "#c5a84f" };
  if (accuracy >= 70)  return { name: "medaille", color: "#c0c0c0" };
  if (accuracy >= 40)  return { name: "medaille", color: "#cd7f32" };
  return { name: "bible", color: "#9ca3af" };
}
...
function BadgeToast({ badge, onDone }: { badge: Badge; onDone: () => void }) {
  ...
  return (
    <div className="fixed bottom-6 right-4 z-[100] flex items-center gap-3 px-4 py-3 rounded-2xl border shadow-2xl"
      ...
      <span className="text-2xl"><Icon name={badge.icon} size={24} /></span>
      <div>
...
const BIBLICAL_FIGURES: BiblicalFigure[] = [
  { name: { fr: "Moïse", ht: "Moyiz", en: "Moses", es: "Moisés" }, emoji: "🗿",
  ...
  { name: { fr: "David", ht: "Davit", en: "David", es: "David" }, emoji: "👑",
  ...
  { name: { fr: "Jésus", ht: "Jezi", en: "Jesus", es: "Jesús" }, emoji: "✝️",
  ...
  { name: { fr: "Paul", ht: "Pòl", en: "Paul", es: "Pablo" }, emoji: "✍️",
  ...
  { name: { fr: "Josué", ht: "Jozye", en: "Joshua", es: "Josué" }, emoji: "⚔️",
  ...
  { name: { fr: "Esther", ht: "Estè", en: "Esther", es: "Ester" }, emoji: "🌹",
  ...
  { name: { fr: "Élie", ht: "Eliy", en: "Elijah", es: "Elías" }, emoji: "🔥",
  ...
  { name: { fr: "Daniel", ht: "Danyèl", en: "Daniel", es: "Daniel" }, emoji: "🦁",
  ...
  return (
    <div className="fixed inset-x-4 bottom-24 z-[90] pointer-events-none"
      ...
      <div className="rounded-2xl border px-5 py-4 flex items-start gap-4 shadow-2xl pointer-events-auto cursor-pointer"
        ...
        onClick={onClose}>
        <span className="text-4xl shrink-0 mt-0.5" style={{ filter: `drop-shadow(0 0 12px ${figure.color}80)` }}>
          {figure.emoji}
        </span>
...
export default function PlayPage() {
  ...
  const globalTimerBadge = gMin !== null && gSec !== null ? (
    <div className={`fixed top-3 right-3 z-50 flex items-center gap-1 px-3 py-1 rounded-full text-[11px] font-black tabular-nums border transition-all ${
      ...
      <Icon name="horloge" size={13} /> {pad(gMin)}:{pad(gSec)}
    </div>
  ) : null;
  ...
  if (phase === "loading" || error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6" style={{ background: "linear-gradient(160deg, #030d1f 0%, #020717 100%)" }}>
        ...
        {error ? (
          <>
            <div className="flex justify-center"><Icon name="alerte" size={48} color="#f59e0b" /></div>
            <p className="text-white/50 text-sm text-center max-w-xs px-5">{error}</p>
            <button onClick={() => router.replace(`/concours/${id}`)} className="text-[#c5a84f] text-sm font-bold">← {l === "fr" ? "Retour" : l === "ht" ? "Retounen" : l === "es" ? "Volver" : "Back"}</button>
          </>
        ) : (
          <>
            <div className="relative">
              <div className="w-16 h-16 rounded-full border-2 border-[#c5a84f]/20 flex items-center justify-center">
                <span className="flex items-center justify-center" style={{ animation: "kp-spin 3s linear infinite" }}><Icon name="croix" size={30} color="#c5a84f" /></span>
              </div>
        ...
        <div className="relative z-10 flex flex-col items-center text-center gap-7 max-w-sm w-full">
          ...
          <div className="relative" style={{ animation: "kp-champ-in .6s cubic-bezier(0.34,1.56,0.64,1) both" }}>
            ...
            <div className="w-24 h-24 rounded-3xl flex items-center justify-center"
              ...
              <span className="flex items-center justify-center" style={{ filter: "drop-shadow(0 0 16px rgba(197,168,79,0.9))", animation: "kp-spin 10s linear infinite" }}><Icon name="croix" size={48} color="#c5a84f" /></span>
            </div>
          </div>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center px-5 relative overflow-hidden"
        ...
        <div className="relative z-10 flex flex-col items-center text-center gap-5 max-w-md w-full">
          ...
          <div className="flex gap-2 flex-wrap justify-center mt-1"
            ...
            {[
              { icon: "info" as IconName, val: `${currentRound.questions.length} question${currentRound.questions.length > 1 ? "s" : ""}` },
  ...
  if (phase === "round_results" && currentRoundResult) {
    ...
    const medal = getMedalIcon(r.accuracy);
      ...
      <div className="min-h-screen flex flex-col items-center justify-center px-5 relative overflow-hidden"
        ...
        <div className="relative z-10 w-full max-w-sm flex flex-col items-center gap-5">
          ...
          <div className="text-center" style={{ animation: "kp-bounce-in .6s cubic-bezier(.34,1.56,.64,1) forwards" }}>
            <div className="mb-2 flex justify-center"><Icon name={medal.name} size={64} color={medal.color} /></div>
            ...
            {perfect && (
              <p className="text-white font-black text-xl mt-1" style={{ animation: "kp-fade-up .4s .3s ease-out both" }}>
                <span className="inline-flex items-center justify-center gap-1.5">{l === "fr" ? "PARFAIT !" : l === "ht" ? "PAFÈ !" : l === "es" ? "¡PERFECTO!" : "PERFECT!"}<Icon name="etincelles" size={18} /></span>
              </p>
            )}
          </div>
          ...
          <button onClick={advanceToNextRound}
            ...
            {isLast ? <span className="inline-flex items-center justify-center gap-2"><Icon name="trophee" size={16} />{l === "fr" ? "Voir mon résultat final" : l === "ht" ? "Wè rezilta final mwen" : l === "es" ? "Ver mi resultado final" : "See my final result"}</span> : (l === "fr" ? "Continuer →" : l === "ht" ? "Kontinye →" : l === "es" ? "Continuar →" : "Continue →")}
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 relative overflow-hidden"
        ...
        <div className="relative z-10 flex flex-col items-center text-center gap-4">
          <div className="flex justify-center" style={{ animation: "kp-bounce-in .7s cubic-bezier(.34,1.56,.64,1) forwards" }}><Icon name="trophee" size={80} color="#c5a84f" /></div>
  ...
  function renderTFOptions() {
    const choices = [
      { label: l === "fr" ? "VRAI" : l === "ht" ? "VRÈ" : l === "es" ? "VERDADERO" : "TRUE",  icon: "valider" as IconName, value: true,  color: "#22c55e" },
      { label: l === "fr" ? "FAUX" : l === "ht" ? "FO"  : l === "es" ? "FALSO" : "FALSE", icon: "fermer" as IconName, value: false, color: "#ef4444" },
    ];
    return (
      <div className="flex flex-col gap-4 w-full">
        {choices.map(({ label, icon, value, color }) => {
          ...
          return (
            <button key={label} onClick={() => handleTFAnswer(value)}
              ...
              <span className="inline-flex items-center justify-center gap-2"><Icon name={icon} size={20} /> {label}</span>
            </button>
          );
        })}
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "#020717" }}>
      ...
      <div className="sticky top-0 z-40 border-b border-white/5"
        ...
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full border text-xs font-black tabular-nums shrink-0 transition-all ${timerCritical ? "animate-pulse" : ""}`}
            ...
            <Icon name="chrono" size={14} /> {roundTimeLeft}s
          </div>
      ...
      <button
        ...
        <Icon name={musicOn ? "audio" : "muet"} size={18} />
      </button>
    </div>
  );
}
