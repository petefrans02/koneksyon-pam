            ...
            <>
              ...
              {champion.total_correct >= 70 && (
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-amber-400/30"
                  ...
                  <Icon name="trophee" size={16} color="#f59e0b" />
    ...
    <div className="animate-page-awaken min-h-screen flex flex-col"
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center px-4 py-10 gap-8">
        ...
        <div className="text-center" style={{ animation: "podium-rise 0.8s ease both" }}>
          ...
          <p className="text-white/30 text-sm mt-2 inline-flex items-center gap-1.5">
            {l === "fr" ? "Merci à tous les participants" : l === "ht" ? "Mèsi a tout patisipan yo" : l === "es" ? "Gracias a todos los participantes" : "Thank you to all participants"}
            <Icon name="priere" size={14} />
          </p>
          {/* Live indicator */}
              ...
              {[top3[1], top3[0], top3[2]].map((p, vi) => {
                ...
                return (
                  <div key={vi} className="flex flex-col items-center gap-1">
                    <span className="text-2xl" style={{ animation: `badge-pop 0.5s ${0.3 + vi * 0.15}s ease both`, opacity: 0, animationFillMode: "forwards" }}>
                    ...
                    {isFirst && p.total_correct >= 70 && (
                      <p className="text-amber-400 text-[9px] font-black text-center inline-flex items-center gap-1"><Icon name="trophee" size={11} /> {l === "fr" ? "Parfait" : l === "ht" ? "Pafè" : l === "es" ? "¡Perfecto!" : "Perfect!"}</p>
                    )}
        ...
        <div className="w-full max-w-sm rounded-3xl border border-white/8 p-6"
          ...
          <button onClick={handleShare}
            className="w-full mt-5 py-3 rounded-2xl border font-bold text-sm transition-all active:scale-95"
            ...
            {shared
              ...
              : <span className="inline-flex items-center gap-2"><Icon name="partage" size={16} />{l === "fr" ? "Partager mon résultat" : l === "ht" ? "Pataje rezilta mwen" : l === "es" ? "Compartir mi resultado" : "Share my result"}</span>}
          </button>
        </div>
              ...
              {leaderboard!.entries.slice(0, 10).map((entry, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3 rounded-2xl px-4 py-3 border transition-all"
                    ...
                    <span className="w-6 text-center text-xs font-black inline-flex items-center justify-center" style={{ color }}>
                      {i < 3 ? <Icon name="medaille" size={14} color={medalColors[i]} /> : `#${entry.rank}`}
                    </span>
                    {entry.player_avatar ? (
                    ...
                    {(entry as PodiumEntry).total_correct >= 70 && (
                      <Icon name="trophee" size={11} color="#f59e0b" />
                    )}
        ...
        <div className="flex flex-col items-center gap-3 mt-2 pb-8"
          ...
          <Link href={`/concours/${id}/certificate`} className="w-full max-w-sm text-center py-4 rounded-3xl font-black text-base transition-all active:scale-95"
            style={{ background: "linear-gradient(135deg, #f59e0b, #d97706)", color: "#020717", boxShadow: "0 0 30px rgba(245,158,11,0.25)" }}>
            <span className="inline-flex items-center justify-center gap-2"><Icon name="medaille" size={18} />{l === "fr" ? "Voir mon certificat" : l === "ht" ? "Wè sètifika mwen" : l === "es" ? "Ver mi certificado" : "View my certificate"}</span>
          </Link>
          <Link href="/concours" className="w-full max-w-sm flex items-center justify-center gap-2 py-3.5 rounded-3xl font-black text-sm transition-all active:scale-95"
