...
export default function LobbyPage() {
  ...
  const launchChampionship = useCallback(() => {
    ...
    setLaunched(true);
    stopMusic();
    router.push(`/concours/${id}/championship`);
  }, [id, router]);
  ...
  if (rejoinBlocked) {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center gap-6 px-6 text-center" style={{ background: "#020717" }}>
        <div className="flex justify-center"><Icon name="deconnexion" size={48} color="#e5e7eb" /></div>
    ...
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center gap-6" style={{ background: "#020717" }}>
        ...
        <div key={countdownSec}
          ...
          {countdownSec <= 0 ? <Icon name="feu" size={90} color="#ef4444" /> : countdownSec}
        </div>
    ...
    <div className="animate-page-awaken min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-8 gap-7 max-w-lg mx-auto w-full">
        ...
        <div className="relative w-16 h-16 flex items-center justify-center" style={{ animation: "lobby-fade-up 0.7s ease both" }}>
          <div className="select-none flex items-center justify-center" style={{ animation: "lobby-pulse 3s ease-in-out infinite" }}><Icon name="croix" size={30} color="#f59e0b" /></div>
          {[0, 120, 240].map(deg => (
        ...
        <div className="w-full" style={{ animation: "lobby-fade-up 0.7s 0.4s ease both", opacity: 0 }}>
          ...
          <div className="flex justify-center gap-1.5 flex-wrap">
            {ROUND_DOTS.map((color, i) => (
              <div key={i} title={ROUND_ICONS[i]}
                className="w-7 h-7 rounded-full flex items-center justify-center text-[13px] border border-white/10 transition-all"
        ...
        <div className="relative w-full flex flex-col items-center gap-3"
          ...
          <div className="flex gap-3 relative">
            {/* Floating reactions */}
            {reactions.map(r => (
              <div key={r.id} className="absolute pointer-events-none text-2xl z-10"
                style={{
            ...
            {EMOJIS.map(emoji => (
              <button key={emoji} onClick={() => sendReaction(emoji)}
                className="w-11 h-11 rounded-full border border-white/10 text-xl flex items-center justify-center transition-all active:scale-90 hover:border-white/25 hover:scale-105"
        ...
        <button onClick={handleAudio}
          ...
          <span style={{ display: "inline-flex" }}><Icon name={audioOn ? "audio" : "muet"} size={18} /></span>
          <span>{audioOn
