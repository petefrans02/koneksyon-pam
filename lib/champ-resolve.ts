...
function countFor(rounds: SubRound[], seasonId: string, stage: string, round_number: number | null): number {
  return flattenForBroadcast(subsetForSeed(rounds, `${seasonId}:${waveKeyOf(stage, round_number)}`)).length || 15;
}
...
export async function resolveMatch(db: SupabaseClient, match: ChampMatch, count?: number): Promise<{ score_a: number; score_b: number; winner: string | null }> {
  ...
  if (count === undefined) { const r = await seasonRounds(db, match.season_id); count = countFor(r, match.season_id, match.stage, match.round_number); }
...
export async function autoResolveReady(db: SupabaseClient, seasonId: string): Promise<number> {
  ...
  for (const m of liveMatches) {
    const count = countFor(rounds, seasonId, m.stage as string, (m.round_number as number) ?? null);
