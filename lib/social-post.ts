...
async function getFacebookPages(): Promise<{ id: string; token: string; name: string }[]> {
  // Primary: read from Supabase (set via OAuth flow — permanent tokens)
  try {
    ...
    if (data && data.length > 0) {
      return data.map(r => ({ id: r.page_id, token: r.access_token, name: r.page_name }));
    }
  } catch {}

  // Fallback: read from environment variables (legacy)
  const pages: { id: string; token: string; name: string }[] = [];
